    ...
    def __init__(self):
        ...
        try:
            self.dist, self.codename = tuple(
                utf8_decode(s).strip() for s in check_output(RELEASE_COMMAND).split()
            )
        except OSError:
...
class Sources(object):
    ...
    def __init__(self, codename):
        ...
        if not path.isfile(self._CONFIG_PATH):
            raise SourcesFileError(("%s must exist as file" % self._CONFIG_PATH))
    ...
    def __confirm_apt_source_uri(self, uri):
        ...
        if (
            uri
            and (uri[0] in self.DEB_SCHEMES)
            and uri[1].split("://")[0] in self.PROTOCOLS
        ):
            return True
        ...
        if not urls:
            raise SourcesFileError(
                (
                    ...
                    % (self._required_component, self._CONFIG_PATH, self.skip_gen_msg)
                )
            )
